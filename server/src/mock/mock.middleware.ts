...
export class MockMiddleware implements NestMiddleware {
  ...
  async use(req: Request, res: Response, next: NextFunction): Promise<void> {
    ...
    const endpointResult = await this.storage.findByPathAndMethod(
      req.path,
      req.method,
      requestedStatusCode,
    );
    ...
    const count =
      typeof endpointResult.count === 'number' ? endpointResult.count : 5;
    ...
    for (const key of keys) {
      ...
      if (
        ...
        result[key] = Array.from({ length: count }, (_, i) =>
          this.generateMockData(subSchema, count, i),
        );
      } else if (value === 'id') {
